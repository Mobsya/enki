...
/*!	\mainpage Enki Reference documentation
    ...
    The core, the IRSensor, and the basic Khepera, EPuck, Alice and Sbot features reflect real hardware and thus won't
change much.
...
//! Enki is the namespace of the Enki simulator. All Enki functions and classes excepted the math one are inside this
//! namespace
namespace Enki {
class World;
...
public:  // inner classes
    ...
    //! Elasticity of collisions of this object. If 0, soft collision, 100% energy dissipation; if 1, elastic collision,
    //! 0% energy dissipation. Actual elasticity is the product of the elasticity of the two colliding objects. Walls
    //! are fully elastics
    ...
    //! The viscous friction moment coefficient. Premultiplied by momentOfInertia. A value of k applies a force of -k *
    //! speed * momentOfInertia
    ...
    public:
        ...
        inline const Polygon& getShape() const {
            return shape;
        }
        inline const Polygon& getTransformedShape() const {
            return transformedShape;
        }
        inline const Point& getCentroid() const {
            return centroid;
        }
        inline const Point& getTransformedCentroid() const {
...
public:  // methods
    ...
    inline bool isCylindric() const {
        return hull.empty();
    }
    inline const Hull& getHull() const {
        return hull;
    }
    ...
    inline double getMomentOfInertia() const {
        return momentOfInertia;
    }
    inline double getInterlacedDistance() const {
        return interlacedDistance;
    }
    ...
    enum MouseButtonCode { MOUSE_BUTTON_LEFT = 0, MOUSE_BUTTON_RIGHT = 1, MOUSE_BUTTON_MIDDLE = 2 };
...
private:  // setup methods
    ...
    void dirtyUserData();
    //! Compute the moment of inertia tensor depending on radius, mass, height, and hull, assuming that the hull is
    //! centered around the center of mass, which is done by setupCentorOfMass()
...
protected:  // physical actions
    /*//! A physics simulation step for this object. It is considered as deinterlaced. The position and orientation are
    updated. virtual void physicsStep(double dt);*/
    ...
    //! The object collided with o during the current physical step, if o is null, it collided with walls. Called just
    //! before the object is de-interlaced
...
public:
    ...
    void addGlobalInteraction(GlobalInteraction* gi) {
        globalInteractions.push_back(gi);
    }
    ...
    //! Sort local interactions. Called by addLocalInteraction ; can be called by subclasses in case of interaction
    //! radius change.
...
public:
    ...
    struct GroundTexture {
        ...
        //! empty if there is no ground texture
...
public:
    ...
    //! Simulate a timestep of dt. dt should be below 1 (typically .02-.1); physicsOversampling is the amount of time
    //! the physics is run per step, as usual collisions require a more precise simulation than the sensor-motor loop
    //! frequency.
    virtual void step(double dt, unsigned physicsOversampling = 1);
    //! Add an object to the world, simply add it to the vector. Object will be automatically deleted when world will be
    //! destroyed. If the object is already in the world, do nothing
    ...
    //! Set to 0 the userData member of all object whose value userData->deletedWithObject are false; call this before
    //! the creator of user data is destroyed, this method is typically called from a viewer just before its
    //! destruction.
