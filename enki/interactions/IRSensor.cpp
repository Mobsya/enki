...
IRSensor::IRSensor(Robot* owner, Vector pos, double height, double orientation, double range, double m, double x0,
                   double c, double noiseSd)
    : pos(pos)
    , height(height)
...
void IRSensor::init(double, World*) {
...
void IRSensor::objectStep(double, World*, PhysicalObject* po) {
...
void IRSensor::wallsStep(double, World* w) {
    switch(w->wallsType) {
        case World::WALLS_SQUARE: {
            // if radius from center point of rays is not touching walls, don't bother
            if((absSmartPos.x - smartRadius > 0) && (absSmartPos.y - smartRadius > 0) &&
               (absSmartPos.x + smartRadius < w->w) && (absSmartPos.y + smartRadius < w->h))
                return;
...
void IRSensor::finalize(double, World*) {
