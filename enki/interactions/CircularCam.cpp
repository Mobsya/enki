...
CircularCam::CircularCam(Robot* owner, Vector pos, double height, double orientation, double halfFieldOfView,
                         unsigned pixelCount)
    : zbuffer(pixelCount), image(pixelCount) {
...
void CircularCam::objectStep(double, World*, PhysicalObject* po) {
    ...
    if(!po->isCylindric()) {
        ...
        const size_t firstPixelUsed =
            static_cast<size_t>(floor((zbuffer.size() - 1) * 0.5 * (beginAngle / halfFieldOfView + 1)));
        const size_t lastPixelUsed =
            static_cast<size_t>(ceil((zbuffer.size() - 1) * 0.5 * (endAngle / halfFieldOfView + 1)));
...
void CircularCam::init(double, World* w) {
...
void CircularCam::wallsStep(double, World* w) {
    ...
    switch(w->wallsType) {
        ...
        case World::WALLS_CIRCULAR: {
            ...
            for(int i = 0; i < segmentCount; ++i) {
                ...
                drawTexturedLine(Point(cos(angStart) * r, sin(angStart) * r), Point(cos(angEnd) * r, sin(angEnd) * r),
                                 texture);
            }
        } break;
...
void CircularCam::finalize(double, World*) {
    if(useFog) {
