...
ViewerWidget::CameraPose::CameraPose() : altitude(0), yaw(0), pitch(0) {}
...
ViewerWidget::CameraPose::CameraPose(const QPointF& pos, double altitude, double yaw, double pitch)
    : pos(pos), altitude(altitude), yaw(yaw), pitch(pitch) {}

ViewerWidget::UpdatableCameraPose::UpdatableCameraPose() : userYaw(0), radius(20) {
    update();
}

ViewerWidget::UpdatableCameraPose::UpdatableCameraPose(const World* world)
    : CameraPose(world), userYaw(M_PI / 2), radius(20) {
    update();
}

ViewerWidget::UpdatableCameraPose::UpdatableCameraPose(const QPointF& pos, double altitude, double yaw, double pitch)
    : CameraPose(pos, altitude, yaw, pitch), userYaw(yaw), radius(20) {
    update();
}
...
void ViewerWidget::UpdatableCameraPose::updateTracking(double targetAngle, const QVector3D& targetPosition,
                                                       double zNear) {
...
ViewerWidget::InfoMessage::InfoMessage(const QString& message, double persistance, const QColor& color,
                                       const QUrl& link)
    : message(message), persistance(persistance), color(color), link(link) {}
...
ViewerWidget::ViewerWidget(World* world, QWidget* parent)
    ...
    elapsedTime =
        double(30) / 1000.;  // average second between two frames, can be updated each frame to better precision
...
/*!
    ...
    This function add an extended attribute to an object using an association table, so it's to the user responsibility
   to delete the extended parameter if object is remove to the world, using the "removeExtendedAttributes" function
*/
...
void ViewerWidget::renderShape(const Polygon& shape, const double height, const Color& color) {
    ...
    for(size_t i = 0; i < segmentCount; ++i) {
        ...
        renderInterSegmentShadow(shape[i], shape[(i + 1) % segmentCount], shape[(i + 2) % segmentCount], height);
    }
...
void ViewerWidget::displayObjectHook(PhysicalObject*) {}
...
void ViewerWidget::initializeGL() {
    ...
    if(world->hasGroundTexture()) {
        ...
        glBindTexture(GL_TEXTURE_2D, worldGroundTexture);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, world->groundTexture.width, world->groundTexture.height, 0, GL_BGRA,
                     GL_UNSIGNED_BYTE, &world->groundTexture.data[0]);
...
void ViewerWidget::renderScene(double left, double right, double bottom, double top, double zNear, double zFar) {
    ...
    for(World::ObjectsIterator it = world->objects.begin(); it != world->objects.end(); ++it) {
        ...
        if(!(*it)->userData) {
            bool found = false;
            const std::type_info* typeToSearch = &typeid(decltype(**it));
...
void ViewerWidget::picking(double left, double right, double bottom, double top, double zNear, double zFar) {
    ...
    for(World::ObjectsIterator it = world->objects.begin(); it != world->objects.end(); ++it) {
        const Vector distOCtoOC =
            (*it)->pos - cursor2Dpoint;  // distance between object bounding circle center and pointed point
        ...
        if(distOCtoOC.norm2() <= (addedRay * addedRay))             // cursor point colide bounding circle
        {
            if(!(*it)->getHull().empty())  // check pointer circle and object hull
            {
                ...
                for(PhysicalObject::Hull::const_iterator it2 = hull.begin(); it2 != hull.end();
                    ++it2)  // check all convex shape of hull
                {
...
void ViewerWidget::paintGL() {
    ...
    if(trackingView && selectedObject)
        camera.updateTracking(selectedObject->angle,
                              QVector3D(selectedObject->pos.x, selectedObject->pos.y, selectedObject->getHeight()),
                              znear);
    else
        camera.update();
...
void ViewerWidget::mousePressEvent(QMouseEvent* event) {
    ...
    if(event->button() == Qt::LeftButton) {
        if(event->x() > width() - 72 && event->x() < width() - 24) {
            ...
            if(messageIndex >= 0 && size_t(messageIndex) < messageList.size()) {
    ...
    if(robot) {
        ...
        if(event->button() == Qt::LeftButton) {
            robot->mousePressEvent(PhysicalObject::MOUSE_BUTTON_LEFT, pointedPointXY.x, pointedPointXY.y,
                                   pointedPoint.z());
            mouseLeftButtonRobot = robot;
        }
        if(event->button() == Qt::RightButton) {
            robot->mousePressEvent(PhysicalObject::MOUSE_BUTTON_RIGHT, pointedPointXY.x, pointedPointXY.y,
                                   pointedPoint.z());
            mouseRightButtonRobot = robot;
        }
        if(event->button() == Qt::MiddleButton) {
            robot->mousePressEvent(PhysicalObject::MOUSE_BUTTON_MIDDLE, pointedPointXY.x, pointedPointXY.y,
                                   pointedPoint.z());
...
void ViewerWidget::mouseMoveEvent(QMouseEvent* event) {
    if(!trackingView && selectedObject) {
        ...
        // rotate
            ...
            camera.pitch = clamp(camera.pitch - sensitivity * (double)diff.y() / (1 + height()), -M_PI / 2 + delta,
                                 M_PI / 2 - delta);
